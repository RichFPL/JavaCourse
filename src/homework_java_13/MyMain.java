...
import static homework_java_13.AuthValidator.validate;

public class MyMain {
    public static void main(String[] args){

        try {
            boolean result = validate("user123", "P@sswOrd1", "P@sswOrd1");
            System.out.println("Валидация user123 успешна: " + result);
        } catch (WrongLoginException | WrongPasswordException e) {
            System.out.println("Ошибка валидации user123: " + e.getMessage());
        }

        System.out.println("------------------------------------");

        try {
            validate("thisIsAVeryLongLoginWhichIsMoreThan20Chars", "P@sswOrd1", "P@sswOrd1");
        } catch (WrongLoginException | WrongPasswordException e) {
            System.out.println("Ошибка валидации длинного логина: " + e.getMessage());
        }

        System.out.println("------------------------------------");

        try {
            validate("user123", "Nodigits", "Nodigits");
        } catch (WrongLoginException | WrongPasswordException e) {
            System.out.println("Ошибка валидации пароля без цифр: " + e.getMessage());
        }
